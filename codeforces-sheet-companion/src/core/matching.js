...
(function (root, factory) {
  ...
  function classifyProblem(problem, index, context, options) {
    ...
    // Submission history only grows, so the two directions of this evidence
    // have different shelf lives. An accepted submission seen in the listing
    // stays accepted forever, so `provenHere` needs no freshness check. Absence
    // is different: a listing read before the user solved the problem is silent
    // about a solve made afterwards. Treating that silence as proof is what
    // turned a just-solved problem into "Solved elsewhere" after a reload, so a
    // listing that is explicitly marked stale (fresh === false) can no longer
    // disprove anything. Proof with no freshness field is treated as fresh, so
    // callers that read the listing synchronously behave as before.
    var disprovenHere = proofUsable && proof.complete && proof.fresh !== false && !provenHere;
    ...
    if (provenHere) {
      ...
      reason = 'accepted in ' + acceptedContexts.join(', ') + ', nothing accepted in ' + here +
        (proofUsable && proof.fresh !== false ? " (checked against this sheet's submissions)" : '');
