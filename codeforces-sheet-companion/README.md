...
The listing is re-read on every load — it is one same-origin GET and it is the
only source that can see a private group's submissions. Absence is only allowed
to disprove "here" when the listing was actually read during this page load
(`fresh !== false`). Submission history only grows, so the two directions of the
evidence have different shelf lives:

| evidence | still valid when cached? |
| --- | --- |
| the problem **is** in `accepted` | yes — an accept never becomes an un-accept |
| the problem is **not** in `accepted` | no — the listing may predate the solve |

That asymmetry is the fix for the reported "solved it, reloaded, still says
Solved elsewhere". A listing cached before the solve reported nothing accepted
here, while the freshly loaded page already showed the row green; the classifier
read that silence as proof and said "elsewhere". Cached proof is now kept only as
a fallback for when the re-read fails, and it is flagged `fresh: false`
(`cfstats:sheet:<handle>:<group>:<contest>`), so at worst a problem reads
**Solved, unknown source** until the listing can be read again — never a
...
Results are cached per handle **and per contest**
(`cfstats:index:<handle>|<contestId>`) for 10 minutes. The contest is part of the
key because the entry merges contest-scoped `contest.status` data; keying on the
handle alone meant that opening sheet A and then sheet B within the TTL answered
B from A's payload, and B's own contest-scoped call was never made.

A cache hit is a candidate answer, not the answer. Every load revalidates it with
one `user.status?count=1` call: because submission history is append-only, an
unchanged newest submission id **proves** nothing has been submitted since the
entry was built, and the entry is reused. A different id means you have submitted
something, and the history is paged again. So the steady-state cost of a reload
is a single call, and a new solve is always picked up on the next load without
any timeout or delay. If the probe itself fails, the cache is still served but
flagged unvalidated. **Refresh** in the section footer skips straight to a full
refetch.

Ratings and tags come from the `problem`
