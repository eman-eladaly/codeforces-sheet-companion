...
function loadIndex({ handle, contestId, force }) {
  const key = String(handle).toLowerCase() + '|' + (contestId || '') + '|' + (force ? 'force' : 'cache');
  if (inFlight.has(key)) return inFlight.get(key);
  const p = api.loadIndex({ handle, contestId, force }).finally(() => inFlight.delete(key));
  inFlight.set(key, p);
  return p;
}
